...
namespace voxblox {
  ...
  void serializeLayerAsOctomapMsg(const Layer<VoxelType> &layer,
                                  octomap_msgs::Octomap *msg, double surface_distance_threshold_factor, const std::string& id_) {
    octomap::OcTree octree;
    serializeLayerAsOctomap(layer, &octree, surface_distance_threshold_factor);
  ...
  template<typename VoxelType>
  void serializeLayerAsOctomap(
      const Layer<VoxelType> &layer, octomap::OcTree *octree, double surface_distance_threshold_factor) {
    ...
    for (const BlockIndex &index : blocks) {
      ...
      for (size_t linear_index = 0; linear_index < num_voxels_per_block;
        ...
        octree->updateNode(coord.x(), coord.y(), coord.z(), occupied);
      }
    }
  }
